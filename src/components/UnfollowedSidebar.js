...
function UnfollowedSidebar({ unfollowedList, onRestoreToQueue }) {
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex justify-between items-end mb-3 border-b border-slate-200 pb-2 flex-shrink-0">
        ...
        <span className="text-slate-500 text-sm font-semibold">{unfollowedList.length}</span>
      </div>
        ...
        <div className="flex flex-col gap-2">
          {unfollowedList.length === 0 ? (
            <div className="p-4 text-center text-xs md:text-sm text-slate-400">
              No accounts unfollowed yet.
            </div>
          ) : (
            unfollowedList.map((user) => (
              <UnfollowedItem
                key={user.username}
                user={user}
...
function UnfollowedItem({ user, onRestore }) {
...
export default UnfollowedSidebar;